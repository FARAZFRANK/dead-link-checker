...
class BLC_Admin
{
    ...
    {
        add_menu_page(
            __('Broken Link Checker', 'dead-link-checker'),
    ...
    public function ajax_export_links()
    {
        ...
        wp_send_json_error(__('Export is a Pro feature. Upgrade to unlock!', 'dead-link-checker'));
    }
    ...
    public function admin_notices()
    {
        ...
        if ($stats['broken'] > 0) {
            ...
            printf('<div class="notice notice-error"><p><strong>%s</strong> %s</p></div>', sprintf(esc_html__('%d broken links detected!', 'dead-link-checker'), intval($stats['broken'])), esc_html__('Review and fix them.', 'dead-link-checker'));
        }
    }
    ...
    public function activation_redirect()
    {
        if (get_transient('blc_activation_redirect')) {
            ...
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- This is an activation redirect, not form processing
    ...
    public function add_help_tab()
    {
        ...
        if (!$screen || strpos($screen->id, 'broken-link-checker') === false) {
            return;
        }
        ...
        $screen->add_help_tab(array(
            ...
            'content' => '<h3>' . __('Broken Link Checker', 'dead-link-checker') . '</h3>' .
                ...
                '<ul>' .
                '<li><strong>' . __('Link Scanning - Automatically discover and check all links on your site.', 'dead-link-checker') . '</strong></li>' .
                '<li><strong>' . __('Page Builder Support - Scan links in Elementor, Divi, WPBakery, and Gutenberg.', 'dead-link-checker') . '</strong></li>' .
                '<li><strong>' . __('Email Notifications - Get notified when broken links are found.', 'dead-link-checker') . '</strong></li>' .
                '<li><strong>' . __('Redirect Manager - Create 301/302/307 redirects for broken URLs.', 'dead-link-checker') . '</strong></li>' .
                '</ul>',
        ));
        ...
        $screen->add_help_tab(array(
            ...
            'content' => '<h3>' . __('Getting Started', 'dead-link-checker') . '</h3>' .
                '<ol>' .
                '<li><strong>' . __('Start a Scan - Click the "Scan Now" button to scan your entire website for broken links.', 'dead-link-checker') . '</strong></li>' .
                '<li><strong>' . __('Review Results - View broken, warning, and working links in the dashboard table.', 'dead-link-checker') . '</strong></li>' .
                '<li><strong>' . __('Fix Links - Click "Edit" to update a broken link directly, or use bulk actions.', 'dead-link-checker') . '</strong></li>' .
                '<li><strong>' . __('Dismiss Links - Ignore false positives by clicking "Dismiss".', 'dead-link-checker') . '</strong></li>' .
                '</ol>' .
        ...
        $screen->add_help_tab(array(
            ...
            'content' => '<h3>' . __('Configuration Options', 'dead-link-checker') . '</h3>' .
                '<ul>' .
                '<li><strong>' . __('Scan Post Types - Choose which post types to include in scans.', 'dead-link-checker') . '</strong></li>' .
                '<li><strong>' . __('Check External Links - Enable/disable checking of external URLs.', 'dead-link-checker') . '</strong></li>' .
                '<li><strong>' . __('Email Notifications - Configure when and how to receive alerts.', 'dead-link-checker') . '</strong></li>' .
                '<li><strong>' . __('Connection Timeout - How long to wait for link responses.', 'dead-link-checker') . '</strong></li>' .
                '</ul>',
        ));
        ...
        $screen->set_help_sidebar(
            ...
            '<p><a href="https://awplife.com/" target="_blank">' . __('Support Forums', 'dead-link-checker') . '</a></p>'
        );
    }

    public function add_dashboard_widget()
    {
        wp_add_dashboard_widget('blc_dashboard_widget', __('Broken Link Checker', 'dead-link-checker'), array($this, 'render_dashboard_widget'));
    }
    ...
    {
        $scans = blc()->database->get_scan_history(5); // FREE: Limited to 5
        ?>
            ...
            <div class="blc-help-container" style="max-width: 900px; margin-top: 20px;">
                ...
                <div class="blc-card"
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px;">
                        ...
                        <div style="padding: 10px; background: #f9f9f9; border-radius: 5px;">
                            <strong><?php esc_html_e('Gutenberg Support', 'dead-link-checker'); ?></strong>
                            <p style="margin: 5px 0 0; color: #666;">
                                <?php esc_html_e('Scan links in Gutenberg blocks.', 'dead-link-checker'); ?>
                            </p>
                        </div>
                        <div style="padding: 10px; background: #f9f9f9; border-radius: 5px;">
                            <strong><?php esc_html_e('Posts & Pages', 'dead-link-checker'); ?></strong>
                            <p style="margin: 5px 0 0; color: #666;">
                                <?php esc_html_e('Scan all your posts and pages for broken links.', 'dead-link-checker'); ?>
                            </p>
                        </div>
                        <div style="padding: 10px; background: #f9f9f9; border-radius: 5px;">
                            <strong><?php esc_html_e('Dismiss/Recheck', 'dead-link-checker'); ?></strong>
                            <p style="margin: 5px 0 0; color: #666;">
                                <?php esc_html_e('Manage broken links with dismiss and recheck options.', 'dead-link-checker'); ?>
                            </p>
                        </div>
                        <div style="padding: 10px; background: #f9f9f9; border-radius: 5px;">
                            <strong><?php esc_html_e('Dashboard Stats', 'dead-link-checker'); ?></strong>
                            <p style="margin: 5px 0 0; color: #666;">
                                <?php esc_html_e('Beautiful dashboard with link statistics.', 'dead-link-checker'); ?>
                            </p>
                        </div>
                    </div>
                    <div
                        style="margin-top: 15px; padding: 15px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 5px; color: #fff;">
                        <strong><?php esc_html_e('Want more features?', 'dead-link-checker'); ?></strong>
                        <a href="https://awplife.com/" target="_blank"
                            style="color: #fff; margin-left: 10px;"><?php esc_html_e('Upgrade to Pro →', 'dead-link-checker'); ?></a>
                    </div>
                </div>
